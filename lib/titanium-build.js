var apd = require("atom-package-dependencies"),
  ...
  at = require("atom"),
  configPackage = require("./config-schema"),
  BufferedProcess = at.BufferedProcess,
  File = at.File,
  tiappFile = null,
  isLiveViewEnabled = atom.config.get('titanium-build.LiveViewEnabled'),
  switchAlloy = false,
  switchClassic = false;
...
module.exports = {
  ...
  activate: function() {
    atom.config.onDidChange('titanium-build.ProjectType.Alloy', this.Alloy);
    atom.config.onDidChange('titanium-build.ProjectType.Classic', this.Classic);
    atom.config.onDidChange('titanium-build.LiveViewEnabled', this.toggleLiveView);
    ...
    atom.commands.add("atom-workspace", "titanium-build:new", this.new);
  },

  Alloy: function(e){
    if(switchAlloy){
      setTimeout(function(){
        atom.config.set('titanium-build.ProjectType.Alloy',true);
      },0);
    }
    ...
    if(e.newValue){
      switchAlloy = true;
      switchClassic = false;
      atom.config.set('titanium-build.ProjectType.Classic',false);
    }
  },
  ...
  Classic: function(e){
    ...
    if(switchClassic){
      setTimeout(function(){
        atom.config.set('titanium-build.ProjectType.Classic',true);
      },0);
    }
    ...
    if(e.newValue){
      switchAlloy = false;
      switchClassic = true;
      atom.config.set('titanium-build.ProjectType.Alloy',false);
    }
  },

  toggleLiveView: function(e){
    isLiveViewEnabled=e.newValue;
  },
  // iPhone Simulators
  runIPhone: function() {
        ...
        _.each(result.simulators.ios, function(simulators, os) {
          _.each(simulators, function(simulator) {
            atom.titaniumBuildConfigs.push({
              ...
              "args": ["build", "--log-level", "debug", "--target", "simulator", "-p", "ios", "-C", simulator.udid, isLiveViewEnabled ? "--liveview" : ""]
            });
          });
        });
      }
      ...
      parser: function(result) {
        _.each(result.emulators, function(emulator) {
          atom.titaniumBuildConfigs.push({
            "exec": "titanium",
            "name": emulator.name,
            "args": ["build", "--log-level", "debug", "--target", "emulator", "--platform", "android", "-C", '"' + emulator.id + '"', isLiveViewEnabled ? "--liveview" : ""]
          });
        });
      }
    });
      ...
      parser: function(result) {
        ...
        _.each(result.devices, function(device) {
          atom.titaniumBuildConfigs.push({
            "exec": "titanium",
            "name": device.name,
            "args": ["build", "--log-level", "debug", "--target", "device", "--platform", "android", "-C", device.id, isLiveViewEnabled ? "--liveview" : ""]
          });
        });
      }
    });
  ...
  new: function() {
    ...
    if(atom.config.get('titanium-build.ProjectType.Classic'))
      str = str.concat(" --classic");
    if(atom.config.get('titanium-build.Register App'))
      str = str.concat(" --no-services");

    str = str.concat(" --project-dir " + atom.config.get('titanium-build.Project Path'));
    str = str.concat(" -n " + atom.config.get('titanium-build.App Name'));
    str = str.concat(" -i " + atom.config.get('titanium-build.App ID'));


    atom.titaniumBuildConfigs = [{
      "exec": "appc",
      "name": "Appcelerator new build",
      "args": ["new -t titanium -f -p ios,android" + str]
    }];

    atom.commands.dispatch(atom.views.getView(atom.workspace.getActivePane()), "build:trigger");
    atom.notifications.addSuccess("Project succesfully created");
  }
